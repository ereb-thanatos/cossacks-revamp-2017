...
void ShowProgStr(char* ss, int prog)
{
	if (CurPalette)
	{
		...
		GPS.ShowGP((RealLx - GPS.GetGPWidth(LOAD.GPID, 0)) / 2, (RealLy - GPS.GetGPHeight(LOAD.GPID, 0)) / 2, LOAD.GPID, 0,
		           0);
	}
	else
	{
...
int ProcessResEdit()
{
	...
	ComboBox* CBSTYLE = DSS.addGP_ComboBoxDLX(nullptr, 263, 119, 491 - 263, CBB_GPFILE, 0, 9, 0, &WhiteFont, &YellowFont,
	                                          nullptr);
	...
	ComboBox* CBRESST = DSS.addGP_ComboBoxDLX(nullptr, 263, 171, 491 - 263, CBB_GPFILE, 0, 9, 0, &WhiteFont, &YellowFont,
	                                          nullptr);
	...
	ComboBox* CBRESTOT = DSS.addGP_ComboBoxDLX(nullptr, 263, 223, 491 - 263, CBB_GPFILE, 0, 9, 0, &WhiteFont, &YellowFont,
	                                           nullptr);
...
int MPL_ChooseConnection()
{
	...
	IP2->Visible = 0;
	InputBox* IBIP = MENU.addInputBox(nullptr, 228 + 72, 180 + 222 + dy2 + dy3 + 26 + 13, IPADDR, 64, 260, 20, &YellowFont,
	                                  &WhiteFont);
...
void ShowCentralMessage(char* Message, int GPIDX)
{
	...
	int DX = 0;
	DrawStdBar2((RealLx - L - 64) / 2 - DX, (RealLy - 70) / 2 + 18, (RealLx + L + 64) / 2 - DX, (RealLy - 70) / 2 + 55,
	            GPIDX);
...
bool EnterPassword()
{
	...
	InputBox* IB = DSS.addInputBox(nullptr, RealLx / 2 - 143, RealLy / 2 - 2, SessPassword, 40, 160, 20, &WhiteFont,
	                               &YellowFont);
	IB->Active = 1;
...
void EnterPersonalMessage(char* Nick)
{
	...
	GP_TextButton* OK = DSS.addGP_TextButton(nullptr, X0 + 9, Y0 + 20, GetTextByID("@OK"), BARS.GPID, 15, &WhiteFont,
	                                         &YellowFont);
	OK->OnUserClick = &MMItemChoose;
	OK->UserParam = mcmOk;
	...
	DSS.addGPPicture(nullptr, X0, Y0, BARS.GPID, 14);
	GPS.ShowGP(X0, Y0, BARS.GPID, 17, 0);
	GP_TextButton* CANCEL = DSS.addGP_TextButton(nullptr, X0 + 9, Y0 + 20, GetTextByID("@CANCEL"), BARS.GPID, 15,
	...
	CBOX->param = BARS.GPID;
	InputBox* IB = DSS.addInputBox(nullptr, RealLx / 2 - 143 - 10 - DL, RealLy / 2 - 2, MESSAGE, 120, 290 + DL * 2, 20,
	                               &WhiteFont, &YellowFont);
	IB->Active = 1;
	char cc2[128];
...
bool MPL_WaitingGame(bool Host, bool SINGLE)
{
	...
	for (int i = 0; i < 8; i++)
	{
		if (SINGLE)
		{
			COMPS[i] = MENU.addGP_ComboBox(nullptr, 38, 161 + i * 26, SINGL.GPID, 24 + 8 * (i % 3), 8, 0, &WhiteFont,
			                               &YellowFont, nullptr);
			KICK[i] = nullptr;
		}
		else
		{
			COMPS[i] = MENU.addGP_ComboBox(nullptr, 38, 161 + i * 26, CCOM.GPID, 8 * (i % 3), 8, 0, &WhiteFont, &YellowFont,
			                               nullptr);
			if (Host)
			{
				KICK[i] = MENU.addGP_ComboBox(nullptr, 38, 161 + i * 26, CCOM.GPID, 8 * (i % 3), 8, 0, &WhiteFont, &YellowFont,
				                              nullptr);
				KICK[i]->AddLine("                                                   ");
		...
		if (!SINGLE)
		{
			...
			VersTextShadow[i] = MENU.addTextButton(nullptr, 376 + 12, 161 + 6 + i * 26, "        ", &SmallBlackFont,
			                                       &SmallBlackFont, &SmallBlackFont, 1);
			VersTextShadow[i]->Visible = 0;
			VersText[i] = MENU.addTextButton(nullptr, 376 + 12 - 1, 161 + 6 - 2 + i * 26, "        ", &SmallYellowFont,
			                                 &SmallYellowFont, &SmallYellowFont, 1);
			VersText[i]->Visible = 0;
			ReadyBack[i] = MENU.addGPPicture(nullptr, 408, 161 + i * 26, BTNS.GPID, 15 + (i % 3));
			ReadyBack[i]->Visible = 0;
			ReadyFlow[i] = MENU.addTextButton(nullptr, 332 + 100 - 3, 164 + i * 26, "100%", &YellowFont, &YellowFont,
			                                  &YellowFont, 1);
			...
			AliasID[i] = MENU.addTextButton(nullptr, 332 + 18, 165 + i * 26, "X", &SmallYellowFont, &SmallYellowFont,
			                                &SmallYellowFont, 1);
		...
		else
		{
			...
			AliasID[i] = MENU.addTextButton(nullptr, 358 + 21, 165 + i * 26, "X", &SmallYellowFont, &SmallYellowFont,
			                                &SmallYellowFont, 1);
			AliasBack[i]->OnMouseOver = &ChangeAlias;
			AliasBack[i]->Visible = 0;
			AliasBack[i]->Nation = 0;
		...
		if (SINGLE)
		{
			MNATION[i] = MENU.addGP_ComboBox(nullptr, 203, 161 + i * 26, SINGL.GPID, 8 * (i % 3), 8, 0, &WhiteFont, &YellowFont,
			                                 nullptr);
		}
		else
		{
			MNATION[i] = MENU.addGP_ComboBox(nullptr, 173, 161 + i * 26, BTNS.GPID, 28 + 8 * (i % 3), 8, 0, &WhiteFont,
			                                 &YellowFont, nullptr);
		}
	...
	ADD_OPT_VAL = MENU.addGP_ComboBoxDLX(nullptr, 633 - 65 - 64 + 26, 161 + 26 * 7, 100 + 100 + 64 - 26, LONGBOX.GPID, 0,
	                                     0, 0, &WhiteFont, &YellowFont, nullptr);

	GPPicture* AOPPIC = MENU.addGPPicture(nullptr, 633 - 65 - 64, 161 + 26 * 7, ISYM3.GPID, 2);

	TextButton* AOPTB = MENU.addTextButton(nullptr, 633 - 65 - 64 + 2, 161 + 26 * 5 + 3, GetTextByID("ADDMPOPT"),
	                                       &WhiteFont, &WhiteFont, &WhiteFont, 0);
...
bool MPL_WaitingBattleGame(bool Host, int BattleID)
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		CD_Present[i]->Visible = 0;
		VersTextShadow[i] = MENU.addTextButton(nullptr, 376 + 12 - 12, 161 + 6 + i * 26, "        ", &SmallBlackFont,
		                                       &SmallBlackFont, &SmallBlackFont, 1);
		VersTextShadow[i]->Visible = 0;
		VersText[i] = MENU.addTextButton(nullptr, 376 + 12 - 12 - 1, 161 + 6 - 2 + i * 26, "        ", &SmallYellowFont,
		                                 &SmallYellowFont, &SmallYellowFont, 1);
		VersText[i]->Visible = 0;
		...
		MNAME[i]->Visible = false;
		MNAME[i]->Enabled = false;
		MNATION[i] = MENU.addGP_ComboBox(nullptr, 183, 161 + i * 26, BTNS.GPID, 28 + 8 * (i % 3), 8, 0, &WhiteFont,
		                                 &YellowFont, nullptr);
		MNATION[i]->Hint = GetTextByID("MMCHANGENAT");
		...
		MNATION[i]->FontDx -= 2;
		ReadyFlow[i] = MENU.addTextButton(nullptr, 332 + 100 - 8, 164 + i * 26, "100%", &YellowFont, &YellowFont, &YellowFont,
		                                  1);
		ReadyFlow[i]->Visible = 0;
		ReadyFlow[i]->Enabled = 0;
	}
...
bool ProcessMenuOptions()
{
	...
	//scroll speed (#3)
	VScrollBar* SSP = MMenu.addGP_ScrollBarL(nullptr, X0, 450 + DY + 10 - 13, 8, ScrollSpeed - 2, BTNS.GPID, 12, LX, 12, 0,
	                                         0);
	...
	ComboBox* VMode = MMenu.addGP_ComboBox(nullptr, 240, 240 - 16 - 13, BTNS.GPID, 0, 9, 0, &WhiteFont, &YellowFont,
	                                       nullptr);
	...
	//Game speed mode
	ComboBox* FMode = MMenu.addGP_ComboBox(nullptr, 240, 421 + 10 - 13, BTNS.GPID, 0, 9, 0, &WhiteFont, &YellowFont,
	                                       nullptr);
	FMode->AddLine(SLOWMOD);
	FMode->AddLine(FASTMOD);
	FMode->CurLine = exFMode;

	ComboBox* MUS = MMenu.addGP_ComboBox(nullptr, 240, 240 - 16 + 80 - 3 - 13, BTNS.GPID, 0, 9, 0, &WhiteFont, &YellowFont,
	                                     nullptr);
...
bool ProcessGameOptions()
{
	...
	ComboBox* MUS = MMenu.addGP_ComboBox(nullptr, 240, 240 - 15 + 80 - 3, BTNS.GPID, 0, 9, 0, &WhiteFont, &YellowFont,
	                                     nullptr);
	MUS->AddLine(GetTextByID("NOCD"));
	MUS->AddLine(GetTextByID("NATMUS"));
	MUS->AddLine(GetTextByID("RNDMUS"));
...
int processMainMenu()
{
	...
	MMenu.addTextButton(nullptr, 1024 - GetRLCStrWidth(BuildVersion, &SmallYellowFont1), 748, BuildVersion,
	                    &SmallYellowFont1, &SmallYellowFont1, &SmallYellowFont1, 0);
...
int EnterHi(int* val, int Type)
{
	...
	GP_TextButton* OkBtn = DSY.addGP_TextButton(nullptr, LX / 4 - 75 + 10, LY + 6 + 21, GetTextByID("@OK"), LONGBOX.GPID,
	                                            9, &WhiteFont, &YellowFont);
	OkBtn->UserParam = mcmOk;
	OkBtn->OnUserClick = &MMItemChoose;
	GP_TextButton* CancelBtn = DSY.addGP_TextButton(nullptr, 3 * LX / 4 - 75 + 10, LY + 6 + 21, GetTextByID("@CANCEL"),
	                                                LONGBOX.GPID, 9, &WhiteFont, &YellowFont);
	CancelBtn->UserParam = mcmCancel;
	CancelBtn->OnUserClick = &MMItemChoose;
	...
	TextButton* LoadBtn = DSY.addTextButton(nullptr, 10, 87, GetTextByID("$#HEIGHT"), &YellowFont, &YellowFont,
	                                        &YellowFont, 0);
	...
	ComboBox* CB = DSY.addGP_ComboBoxDLX(nullptr, 120, 44, LX - 140, LONGBOX.GPID, 0, 9, 9, &WhiteFont, &YellowFont,
	                                     nullptr);
...
void EnterChat()
{
	...
	if (ItemChoose == mcmOk)
	{
		...
		else if (!strcmp(passw, "AI") || !strcmp(passw, "ai"))
		{
...
void ChooseUnit()
{
	...
	ListBox* LBU = DSY.addListBox(PIC1, GETV("CUUNITX"), GETV("CUUNITY"), GETV("CUNY"), GETV("CULX"), GETV("CULY"), &BtnL1,
	                              &BtnL0, nullptr);
	ListBox* LBB = DSY.addListBox(PIC1, GETV("CUBUILDX"), GETV("CUBUILDY"), GETV("CUNY"), GETV("CULX"), GETV("CULY"),
	                              &BtnL1, &BtnL0, nullptr);
	LBB->DFont = &SelF;
	VScrollBar* VSU = DSY.addVScrollBar(PIC1, GETV("CUUNITSX"), GETV("CUUNITSY"), 1000, 4, &scr_U, &scr_U, &scr_U, &scr_D,
	                                    &scr_D, &scr_D, &scr_Line, &scr_Line, &scr_V);
	VScrollBar* VSB = DSY.addVScrollBar(PIC1, GETV("CUBUILDSX"), GETV("CUBUILDSY"), 1000, 4, &scr_U, &scr_U, &scr_U,
	                                    &scr_D, &scr_D, &scr_D, &scr_Line, &scr_Line, &scr_V);
	...
	OkBtn->UserParam = mcmOk;
	OkBtn->OnUserClick = &MMItemChoose;
	TextButton* CancelBtn = DSY.addTextButton(PIC1, GETV("CUNOX"), GETV("CUNOY"), GETS("@CUNOSTR"), &BtnF0, &BtnF1, &BtnF1,
	                                          1);
	CancelBtn->UserParam = mcmCancel;
	CancelBtn->OnUserClick = &MMItemChoose;
	Nation* NT = &NATIONS[MyNation];
...
int SelectTexture()
{
	...
	CancelBtn->UserParam = mcmCancel;
	CancelBtn->OnUserClick = &MMItemChoose;
	VScrollBar* VS = GMM.addVScrollBar(PIC1, GETV("STSCRX"), GETV("STSCRY"), (MaxTex >> 2) - 4, 0, &scr_U, &scr_U, &scr_U,
	                                   &scr_D, &scr_D, &scr_D, &scr_Line, &scr_Line, &scr_V);
...
void ProcessHelp()
{
	...
	int LY = 0;
	TextViewer* TV;
	GMM.addTextButton(nullptr, (MPPLX >> 1) + 1, -20, GetTextByID("MISSOBJ"), &BigBlackFont, &BigBlackFont, &BigBlackFont,
	                  1);
	GMM.addTextButton(nullptr, MPPLX >> 1, -21, GetTextByID("MISSOBJ"), &BigYellowFont, &BigYellowFont, &BigYellowFont, 1);
	if (SCENINF.hLib)
	{
		TV = GMM.addTextViewer(nullptr, MPLDX, MPUDY + LY, MPPLX - MPLDX - MPRDX, MPPLY - MPDDY - MPUDY - LY - 40, "miss.txt",
		                       &BlackFont);
	}
	else
	{
		TV = GMM.addTextViewer(nullptr, MPLDX, MPUDY + LY, MPPLX - MPLDX - MPRDX, MPPLY - MPDDY - MPUDY - LY - 40, "help.txt",
		                       &BlackFont);
	};
	TV->SymSize += 3;
...
void ProcessMissionText(char* Bmp, char* Text)
{
	...
	*/
	...
	VScrollBar* SB = GMM.addNewGP_VScrollBar(nullptr, MPPLX - MPRDX + 5 - 4, MPUDY + LY, MPPLY - MPDDY - MPUDY - LY - 40,
	                                         1, 0, SCROL.GPID, 0);
	TextViewer* TV;
	TV = GMM.addTextViewer(nullptr, MPLDX - 6, MPUDY + LY, MPPLX - MPLDX - MPRDX - 6, MPPLY - MPDDY - MPUDY - LY - 40,
	                       "tmp.txt", &f16x16w);
	TV->AssignScroll(SB);
	TextButton* YesBtn = GMM.addTextButton(nullptr, MPPLX >> 1, MPPLY - 60, GetTextByID("@MSOK"), &fntrb, &fntb, &fntb, 1);
	YesBtn->UserParam = mcmOk;
...
int ShowHistryItem(char* Bmp, char* Text)
{
	...
	VScrollBar* SB = GMM.addNewGP_VScrollBar(nullptr, MPPLX - MPRDX + 5 - 6, MPUDY + LY, MPPLY - MPDDY - MPUDY - LY - 40,
	                                         1, 0, SCROL.GPID, 0);
	TextViewer* TV;
	TV = GMM.addTextViewer(nullptr, MPLDX - 6, MPUDY + LY, MPPLX - MPLDX - MPRDX - 6, MPPLY - MPDDY - MPUDY - LY - 40,
	                       "tmp.txt", &f16x16w);
	...
	TextButton* YesBtn = GMM.addTextButton(nullptr, MPPLX / 6, MPPLY - 60, GetTextByID("@MSPREV"), &fntrb, &fntb, &fntb,
	                                       1);
	YesBtn->UserParam = 1;
	YesBtn->OnUserClick = &MMItemChoose;
	...
	TextButton* CBtn = GMM.addTextButton(nullptr, 5 * MPPLX / 6, MPPLY - 60, GetTextByID("@MSCANCEL"), &fntrb, &fntb,
	                                     &fntb, 1);
	CBtn->UserParam = 3;
...
bool AskMissionQuestion(char* Bmp, char* Text)
{
	...
	*/
	...
	TextViewer* TV;
	TV = GMM.addTextViewer(nullptr, MPLDX, MPUDY + LY, MPPLX - MPLDX - MPRDX, MPPLY - LY - MPDDY - MPUDY - 40, "tmp.txt",
	                       &f16x16w);
	ItemChoose = -1;
...
void EditMiniMap()
{
	...
	TRN1->Hint = "Terrain 1";
	CheckBox* TRN3 = EMM.addGP_CheckBox(nullptr, 59, 358 + BTDY, "", &YellowFont, &YellowFont, 1, 0, BTNS.GPID, 11, 12,
	                                    13);
	TRN3->Hint = "Terrain 2";
	CheckBox* TRN4 = EMM.addGP_CheckBox(nullptr, 87, 358 + BTDY, "", &YellowFont, &YellowFont, 1, 0, BTNS.GPID, 14, 15,
	                                    16);
	TRN4->Hint = "Terrain 3";
	CheckBox* TRN5 = EMM.addGP_CheckBox(nullptr, 115, 358 + BTDY, "", &YellowFont, &YellowFont, 1, 0, BTNS.GPID, 17, 18,
	                                    19);
	TRN5->Hint = "Terrain 4";
	CheckBox* TRN6 = EMM.addGP_CheckBox(nullptr, 143, 358 + BTDY, "", &YellowFont, &YellowFont, 1, 0, BTNS.GPID, 20, 21,
	                                    22);
	TRN6->Hint = "Terrain 5";
	...
	CheckBox* Absolut = EMM.addGP_CheckBox(nullptr, 183, 358 + BTDY, "", &YellowFont, &YellowFont, 2, 0, BTNS.GPID, 35, 36,
	                                       37);
	Absolut->Hint = "Absolut";
	CheckBox* Soft = EMM.addGP_CheckBox(nullptr, 211, 358 + BTDY, "", &YellowFont, &YellowFont, 2, 0, BTNS.GPID, 38, 39,
	                                    40);
	Soft->Hint = "Soft";
	CheckBox* More = EMM.addGP_CheckBox(nullptr, 239, 358 + BTDY, "", &YellowFont, &YellowFont, 2, 0, BTNS.GPID, 41, 42,
	                                    43);
	More->Hint = "More";
	CheckBox* Less = EMM.addGP_CheckBox(nullptr, 267, 358 + BTDY, "", &YellowFont, &YellowFont, 2, 0, BTNS.GPID, 44, 45,
	                                    46);
	Less->Hint = "Less";
	...
	CheckBox* PEN1 = EMM.addGP_CheckBox(nullptr, 307, 358 + BTDY, "", &YellowFont, &YellowFont, 3, 0, BTNS.GPID, 23, 24,
	                                    25);
	PEN1->Hint = "Round";
	CheckBox* PEN2 = EMM.addGP_CheckBox(nullptr, 335, 358 + BTDY, "", &YellowFont, &YellowFont, 3, 0, BTNS.GPID, 26, 27,
	                                    28);
	PEN2->Hint = "Rough";
	if (HiRound == 0)PEN1->State = 1;
	if (HiRound == 1)PEN2->State = 1;
	if (HiMode == 0)Absolut->State = 1;
	CheckBox* GETCL = EMM.addGP_CheckBox(nullptr, 375, 358 + BTDY, "", &YellowFont, &YellowFont, 4, 0, BTNS.GPID, 29, 30,
	                                     31);
	GETCL->Hint = "GETCL";
...
void ChooseTree()
{
	...
	VScrollBar* SB = GMM.addHScrollBar(PIC1, 65, 328, RB->MaxXpos, 0, &scr_L, &scr_L, &scr_L, &scr_R, &scr_R, &scr_R,
	                                   &scr_Line, &scr_Line, &scr_U_G);
	...
	{
		for (int z = 0; z < RB->NItems; z++)
			if (RB->Choosed[z] && NTreeItems < 256)
			{
...
bool WaitingGame(bool Host)
{
	...
	TextButton* CancelBtn = MPLAY.addTextButton(nullptr, GETV("MICANCELX"), GETV("MICANCELY"), GETS("@MICANCEL"), &FontA,
	                                            &FontP, &FontP, 1);
	CancelBtn->UserParam = mcmCancel;
	CancelBtn->OnUserClick = &MMItemChoose;
	ItemChoose = -1;
...
StartAI(byte Nat, char* Name, int Land, int Money, int ResOnMap, int Difficulty);
...
bool ProcessSingleCampagin(int n)
{
	...
	for (int i = 0; i < MaxMiss; i++)
	{
		...
		{
			if (Style)TV[i] = MMenu.addTextViewer(nullptr, 54, 431 - 52, 386, 196 - 12, MISSLIST.MISS[SCM->Miss[i]].Intro[0],
			                                      &YellowFont);
...
bool ProcessOneBattle(int BtlID)
{
	...
	if (Preview.PicPtr)
	{
		...
		{
			MMenu.addTextButton(nullptr, 40 + OB->Coor[i + i], 133 + 26 + OB->Coor[i + i + 1], OB->Hints[i], &SmallBlackFont1,
			                    &SmallBlackFont1, &SmallBlackFont1, 0);
			MMenu.addTextButton(nullptr, 38 + OB->Coor[i + i], 133 + 26 + OB->Coor[i + i + 1], OB->Hints[i], &SmallBlackFont1,
			                    &SmallBlackFont1, &SmallBlackFont1, 0);
			MMenu.addTextButton(nullptr, 38 + OB->Coor[i + i], 135 + 26 + OB->Coor[i + i + 1], OB->Hints[i], &SmallBlackFont1,
			                    &SmallBlackFont1, &SmallBlackFont1, 0);
			MMenu.addTextButton(nullptr, 40 + OB->Coor[i + i], 135 + 26 + OB->Coor[i + i + 1], OB->Hints[i], &SmallBlackFont1,
			                    &SmallBlackFont1, &SmallBlackFont1, 0);
			MMenu.addTextButton(nullptr, 39 + OB->Coor[i + i], 134 + 26 + OB->Coor[i + i + 1], OB->Hints[i], &SmallWhiteFont1,
			                    &SmallWhiteFont1, &SmallWhiteFont1, 0);
		};
	};
	VideoButton* OkBtn = MMenu.addVideoButton(nullptr, 862, 568, OK1.GPID, OK2.GPID);
	OkBtn->OnUserClick = &MMItemChoose;
...
int ShowStatScreen(bool Next, bool Prev, byte Kind)
{
	...
	if (NNAT)
	{
		...
		if (ExStyle)
		{
			GP_TextButton* BT_REPLAY = STAT.addGP_TextButton(nullptr, 415, 723, GetTextByID("REPLAY"), BTNS.GPID, 9, &WhiteFont,
			                                                 &YellowFont);
			BT_REPLAY->OnUserClick = &MMItemChoose;
			BT_REPLAY->UserParam = 131;
		}

		GP_TextButton* RGAME = STAT.addGP_TextButton(nullptr, 263 - DDX, 723, GetTextByID("ST_GAM"), BTNS.GPID, 0, &WhiteFont,
		                                             &YellowFont);
		...
		if (PlayGameMode)
		{
			RMENU = STAT.addGP_TextButton(nullptr, 526 + DDX, 723, GetTextByID("ST_MEN"), BTNS.GPID, 0, &YellowFont,
			                              &YellowFont);
			RMENU->Enabled = 0;
		}
		else
...
int ShowUserStat(bool Prev, bool Next)
{
	...
	do
	{
		...
		for (int q = 0; q < 6; q++)
		{
			ShowString(rx1 + rxl * q + rxl + (rxl / 2) - (GetRLCStrWidth(RDS[q].Name, &YellowFont) / 2), ry + 6, RDS[q].Name,
			           &YellowFont);
		}
...
void ShowMultiplayerChat()
{
	if (EnterChatMode)
	{
		...
		ShowString(x0 - GetRLCStrWidth(CHATHELP2, &SmallYellowFont) / 2, y0 + 20 + 7 + DDD / 2 + DDD, CHATHELP2,
		           &SmallYellowFont);
		for (int i = 0; i < NGSPlayers; i++)
		{
...
void CreateMapInterface()
{
	...
	for (int i = 0; i < 22; i++)
	{
		GP_Button* BT = MAPTOOLS.addGP_Button(nullptr, x0 + 13 + (i & 1) * 30, y0 + 42 + (i / 2) * 30, MapGP, 3 + i * 3,
		                                      2 + i * 3);
...
void CreateTOOL_PARAM_UNIT()
{
	...
	CreateFlags(10, 10);
	TP_GPB = TOOL_PARAM.addGP_ComboBoxDLX(nullptr, 10, 10 + 26, 224, CBB_GPFILE, 0, 9, 0, &WhiteFont, &YellowFont,
	                                      nullptr);
	...
	TP_Scroll->ScrDy = 200;
	TP_Scroll->OnesDy = 32;
	TOOL_PARAM.addClipper(TOOL_PARAM.BaseX + 4, TOOL_PARAM.BaseY + 48 + 26 + 1, RealLx,
	                      TOOL_PARAM.BaseY + 311 + 48 + 26 + 1);
	...
	for (int j = 0; j < NUNITS; j++)
	{
		...
		{
			if (xx)yy += 44;
			TextButton* TB = TOOL_PARAM.addTextButton(nullptr, 10, yy, GetTextByID(NATIONS->UnitNames[TP_CurNation][j]),
			                                          &YellowFont, &YellowFont, &YellowFont, 0);
	...
	for (int i = 0; i <= CURSG->NSpr; i++)
	{
		...
		else
		{
			...
			for (int j = i - Nx + 1; j <= i; j++)
			{
				...
				Lx -= dxx;
				Ly -= dyy;
				GPPicture* GPP = TOOL_PARAM.addGPPicture(nullptr, xx + tdx0 + L1 / 2 - dxx, yy - dyy + (MaxLy - Ly) / 2,
				                                         CURSG->FileID, j);
...
void CreateTOOL_PARAM_TEXTURE()
{
	...
	ED_CB0 = TOOL_PARAM.addGP_ComboBoxDLX(nullptr, X0 + maxx, Y0, CMLX, CBB_GPFILE, 0, 9, 0, &YellowFont, &WhiteFont,
	                                      nullptr);
	...
	ED_CB1 = TOOL_PARAM.addGP_ComboBoxDLX(nullptr, X0 + maxx, Y0, CMLX, CBB_GPFILE, 0, 9, 0, &YellowFont, &WhiteFont,
	                                      nullptr);
	...
	ED_CB2 = TOOL_PARAM.addGP_ComboBoxDLX(nullptr, X0 + maxx, Y0, CMLX, CBB_GPFILE, 0, 9, 0, &YellowFont, &WhiteFont,
	                                      nullptr);
	...
	ED_CB3 = TOOL_PARAM.addGP_ComboBoxDLX(nullptr, X0 + maxx, Y0, CMLX, CBB_GPFILE, 0, 9, 0, &YellowFont, &WhiteFont,
	                                      nullptr);
	...
	IB->Enabled = 0;
	Y0 += ODY;
	TP_GPB = TOOL_PARAM.addGP_ComboBoxDLX(nullptr, 0, Y0, InfDX - 15, CBB_GPFILE, 0, 9, 0, &WhiteFont, &YellowFont,
	                                      nullptr);
...
void ProcessTOOL_PARAM_TEXTURE()
{
	...
	if (TM_Angle0 != 90)TM_Tan0 = int(tan(double(TM_Angle0) * 3.1415 / 180) * 256);
	else TM_Tan0 = 100000;
	if (TM_Angle1 != 90)TM_Tan1 = int(tan(double(TM_Angle1) * 3.1415 / 180) * 256);
	else TM_Tan1 = 100000;
	int ntx = 0;
...
void CreateNEWMAP()
{
	...
	GP_TextButton* OKBTN = DSS.addGP_TextButton(nullptr, XB + 9, YB + 20, GetTextByID("@OK"), CBB_GPFILE, 9, &WhiteFont,
	                                            &YellowFont);
	OKBTN->OnUserClick = &MMItemChoose;
	OKBTN->UserParam = mcmOk;
	XB = 180;
	...
	DSS.addGPPicture(nullptr, XB, YB, CBB_GPFILE, 8);
	DSS.addGPPicture(nullptr, XB, YB, CBB_GPFILE, 11);
	GP_TextButton* CANCELBTN = DSS.addGP_TextButton(nullptr, XB + 9, YB + 20, GetTextByID("@CANCEL"), CBB_GPFILE, 9,
	                                                &WhiteFont, &YellowFont);
	CANCELBTN->OnUserClick = &MMItemChoose;
