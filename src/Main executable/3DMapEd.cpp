							...
							case 1: //L
								if (CheckPt(xx, yy))
								{
									...
									if (r && CheckPt(xx - r, yy) && CheckPt(xx + r, yy) && CheckPt(xx, yy - r) && CheckPt(xx, yy + r) &&
										CheckPt(xx - rd, yy - rd) && CheckPt(xx + rd, yy - rd) && CheckPt(xx - rd, yy + rd) && CheckPt(
											xx + rd, yy + rd))
										UNFND = true;
								};
								break;
	...
	if (F)
	{
		...
		do
		{
			...
			if (ccc[0] == '#' && !strcmp(ccc + 1, NationID))
			{
				...
				int z = sscanf(ccc, "%d%d%d%d%d%d", ADDRES + TreeID, ADDRES + FoodID, ADDRES + StoneID, ADDRES + GoldID,
				               ADDRES + IronID, ADDRES + CoalID);
				if (z != 6)
				{
					memset(ADDRES, 0, 8 * 4);
					GSU_Error(NationID);
