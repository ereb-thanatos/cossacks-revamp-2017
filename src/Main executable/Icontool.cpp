...
{
	if ((!NoPress) && ((realLpressed && mouseX < x + IconLx && mouseY < y + IconLy && mouseX >= x && mouseX > x && mouseY >
			y) ||
	...
	{
		if (Disabled)GPS.ShowGPTransparent(x, y, FileID, SpriteID, NatRefTBL[MyNation]);
			//.ShowGPPal(x,y,FileID,SpriteID,MyNation,GraySet);
		else
		{
			if (Type & 512)GPS.ShowGPPal(x, y, FileID, SpriteID, NatRefTBL[MyNation],
			                             /*9+int(8.0*sin(float(GetTickCount())/100)),*/graysc);
