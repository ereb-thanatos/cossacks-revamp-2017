...
#include "Nature.h"
char* WPK[7] = {
	"Удар мечем", "Удар стрелой", "Удар пикой", "Выстрел ядром", "Выстрел", "Выстрел картечью", "Поражение от гранаты"
};
...
void sprintAttack(AdvCharacter* ADC, char* cc)
{
	...
	for (int i = 0; i < 4; i++)
		...
		{
			Nn++;
			if (Nn != Na)sprintf(cc, "%s:%d (радиус действия %d-%d), ", WPK[ADC->WeaponKind[i]], ADC->MaxDamage[i],
			                     ADC->AttackRadius1[i], ADC->AttackRadius2[i]);
			else sprintf(cc, "%s:%d (радиус действия %d-%d)", WPK[ADC->WeaponKind[i]], ADC->MaxDamage[i], ADC->AttackRadius1[i],
			             ADC->AttackRadius2[i]);
			cc += strlen(cc);
		};
};
...
void sprintAttackInComparison(AdvCharacter* ADC, AdvCharacter* MODIF, char* cc)
{
	...
	int Nn = 0;
	for (int i = 0; i < 4; i++)
		if (ADC->MaxDamage[i] != MODIF->MaxDamage[i])
		{
			Nn++;
...
void sprintShieldInComp(AdvCharacter* ADC, AdvCharacter* MODIF, char* cc)
{
	...
	cc += strlen(cc);
	for (int i = 0; i < 7; i++)
		if (ADC->Protection[i] != MODIF->Protection[i])
		{
			Nn++;
...
void PrintAttackUpgrades(Nation* NT, int NIndex, AdvCharacter* START, GFILE* f)
{
	...
	for (int i = 0; i < NT->NUpgrades; i++)
	{
		...
		if (NU->CtgUpgrade == 12 && NU->UnitGroup == NULL && NU->UnitType == 0 && NU->UnitValue == NIndex && NU->CtgGroup ==
			NULL)
		{
			//Upgrade on attack
			Present = true;
...
void MakeReportOnUnit(Nation* NT, int NIndex, GFILE* f)
{
	...
	Gprintf(f, "%s\nЖизнь:%d\nВремя строительства:%d\nЦена:", GO->Message, GO->MoreCharacter->Life,
	        GO->MoreCharacter->ProduceStages);
