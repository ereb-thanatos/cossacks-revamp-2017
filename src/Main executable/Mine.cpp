...
bool OneObject::GoToMine(word ID, byte Prio, byte Type)
{
	...
	if (!(NewMonst && (MiNM->UnitAbsorber ||
		(MiNM->PeasantAbsorber && NM->Peasant))))
		return false;
	...
	if (LocalOrder && LocalOrder->OrderType == 242
		&& LocalOrder->info.BuildObj.ObjIndex == ID)
		return true;
	...
	if (Nsel)
	{
		...
		if (Nsel == 1 || (GO->OFCR && OneType))
		{
			...
			if (GO->OFCR)
			{
				...
				bool are_drummers_nearby = 0 != EnumUnitsInRound(OB->RealX, OB->RealY, MobilR * 16, OFCR->BarabanID,
				                                                 NatRefTBL[MyNation]);
...
void EnumerateScreenSprites(SprInf* SPI)
{
	...
	{
		xs = (cx << 7) + (1 << (7 - 1));
		if (GetRealCoords(xs, ys, &xr, &yr))cy0 = (yr >> 7) - 1;
		else cy0 = 0;
		if (GetRealCoords(xs, ys1, &xr, &yr))cy1 = (yr >> 7) + 1;
		else cy1 = 63;
		if (cy0 < 0)cy0 = 0;
