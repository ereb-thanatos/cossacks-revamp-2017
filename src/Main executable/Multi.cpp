...
void ProcessVotingKeys()
{
	if (VotingMode)
	{
		...
		if (ALLYES && GetTickCount() < LastVotingTime + 60000 * 3 - 4000)
		{
			...
			//All players have accepted the offer. This game will not be rated.
			DontMakeRaiting();
...
void SelectAllUnits(byte NI, bool Re)
{
	...
	{
		OneObject* OB = Group[i];
		if (OB && OB->NNUM == NatRefTBL[NI] && !(OB->LockType || OB->NewBuilding || OB->Sdoxlo || OB->Wall || OB->
			UnlimitedMotion || OB->Guard != 0xFFFF || OB->UnlimitedMotion))N++;
	};
	UNI[0] = new word[N];
	USN[0] = new word[N];
	lpN[0] = N;
	...
	for (int i = 0; i < MAXOBJECT; i++)
	{
		OneObject* OB = Group[i];
		if (OB && OB->NNUM == NatRefTBL[NI] && !(OB->LockType || OB->NewBuilding || OB->Sdoxlo || OB->Wall || OB->
			UnlimitedMotion || OB->Guard != 0xFFFF || OB->UnlimitedMotion))
		{
			SMN[N] = OB->Index;
			SRN[N] = OB->Serial;
			if (Re)OB->Selected |= GM( NI );
...
void ExecuteBuffer()
{
	...
	while (pos < EBPos)
	{
		...
		case 2: //Send group to (X,Y)
			tp = ExBuf[pos + 11];
			if ((tp & 15) < 8)SendSelectedToXY(ExBuf[pos], *(int*)(&ExBuf[pos + 1]), *(int*)(&ExBuf[pos + 5]),
			                                   *(short*)(&ExBuf[pos + 9]), tp);
			else SendSelectedToXY(ExBuf[pos], *(int*)(&ExBuf[pos + 1]), *(int*)(&ExBuf[pos + 5]), *(short*)(&ExBuf[pos + 9]),
			                      128, tp);
			pos += 12;
			break;
