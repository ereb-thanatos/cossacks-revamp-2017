...
#define GE_DATAERROR	5
	...
	**********/
	...
	GServerList ServerListNew(char* gamename, char* enginename, char* seckey, int maxconcupdates, void* CallBackFn,
	                          int CallBackFnType, void* instance);
	...
	If async = true, the function will return immediately, but you must call ServerListThink 
	every ~10ms for list processing and querying to occur. */
	GError ServerListLANUpdate(GServerList serverlist, gbool async, int startsearchport, int endsearchport,
	                           int searchdelta);
