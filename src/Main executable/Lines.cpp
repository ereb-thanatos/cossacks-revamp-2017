...
void Hline(int x, int y, int xend, byte c)
{
	...
	if (xend >= x)
	{
		...
		if (xend > WindX1)Lxr = WindX1 - xr + 1;
		else Lxr = xend - xr + 1;
	}
	else
	{
		if (xend > WindX1)return;
		if (xend < WindX)xr = WindX;
		else xr = xend;
		if (x > WindX1)Lxr = WindX1 - xr + 1;
		else Lxr = x - xr + 1;
	};
...
void Vline(int x, int y, int yend, byte c)
{
	...
	if (yend >= y)
	{
		...
		if (yend > WindY1)Lyr = WindY1 - yr + 1;
		else Lyr = yend - yr + 1;
	}
	else
	{
		if (yend > WindY1)return;
		if (yend < WindY)yr = WindY;
		else yr = yend;
		if (y > WindY1)Lyr = WindY1 - yr + 1;
		else Lyr = y - yr + 1;
	};
