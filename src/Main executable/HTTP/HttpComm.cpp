		...
		case csConnecting:
			{
				...
				if (select(0, NULL, &write_fds, &except_fds, &TimeOut) != SOCKET_ERROR)
				{
					...
					{
						if (send(m_pRequestList[i].m_Socket, m_pRequestList[i].m_lpszQuery, strlen(m_pRequestList[i].m_lpszQuery),
						         0) != SOCKET_ERROR)
						{
		...
		case csSending:
			{
				...
				if (select(0, NULL, &read_fds, &except_fds, &TimeOut) != SOCKET_ERROR)
				{
					...
					{
						if ((iRecvSize = recv(m_pRequestList[i].m_Socket, (LPSTR)m_pRequestList[i].m_lpvBuffer, RECV_BUFFER_SIZE, 0)) !=
							SOCKET_ERROR)
						{
