...
DWORD SXP_DevScope::SendRequest(sicExplorer* SXP, ParsedRQ* P1, bool AllowNewWindow, bool Auto)
{
	...
	for (int i = 0; i < P1->NComm; i++)
		if (!P1->Comm[i].ComID[0])
		{
			P1->DelComm(i);
			i--;
		};
	for (int i = 0; i < NDEVS; i++)
		if (!strcmp(DEVS[i].Name, P1->DevName))
		{
		...
		for (int i = 0; i < NW; i++)
		{
			...
			if (!strcmp(OSW->WinID, win))
			{
				for (int t = 0; t < Np - 1; t++)
				{
					char* box = par[t];
					for (int j = 0; j < OSW->NBoxes; j++)
						if (!strcmp(OSW->Boxes[j].Name, box))
						{
							DialogsSystem* DSS = &OSW->Boxes[j].DSS;
							if (DSS)
							{
			...
			if (!strcmp(OSW->WinID, win))
			{
				for (int t = 0; t < Np - 1; t++)
				{
					...
					for (int j = 0; j < OSW->NBoxes; j++)
					{
						...
						if (DSS)
						{
							for (int p = 0; p < OBX->NISPOS; p++)
								if (!strcmp(OBX->ISPOS[p].ID, box))
								{
...
void exec_LW_dfp(int Np, char** par, int* size)
{
	if (Np >= 2 && size[1] >= 8)
	{
		...
		if (f)
		{
			fwrite(DD + 2, 1, sz, f);
			for (int i = 0; i < N_DNPR; i++)
				if (!strcmp(DNPR[i].HashName, par[0]))
				{
