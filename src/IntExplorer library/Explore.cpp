...
void OneSicWindow::Process()
{
	...
	for (int V = 0; V < NTIME; V++)
	{
		if (TimeLimit[V] && TT - TimeLimit[V] > 0)
		{
			...
			else if (TimeLimitRequest[V][0] == 'o' &&
...
int CReadY(char** sx, int x0, int x1, OneBox* BOX)
{
	...
	while (s[0] != ',' && s[0] != ')' && s[0] != ']' && s[0])
	{
		if (s[0] == '%')
		{
			int p1 = 0;
			char c;
			while ((c = s[p1]) != 0 && c != ',' && c != ']' && c != ')' && c != '+' && c != '-' && c != '[' && c != '(' && p1 <
				14)p1++;
			if (p1 < 14 && BOX)
			{
				char name[16];
...
int OneSicWindow::ParseTheWholeText()
{
	...
	do
	{
		...
		if (c == '#')
		{
						...
						if (GPIDX != -1)
						{
							...
							if (gpid != -1)
							{
								OBX->VS = OBX->DSS.addNewGP_VScrollBar(nullptr,
								                                       OBX->xi1 + sc_dx, OBX->yi + sc_dy, OBX->yi1 - OBX->yi + 1 + sc_dly, 10,
								                                       0, gpid, scr_Idx);
			...
			else if (!strcmp(com, "#large"))SetChatWState(1, 1);
			else if (!strcmp(com, "#small"))SetChatWState(0, 1);
			else if (!strcmp(com, "#lockresize"))SetChatWState(2, 1);
			else if (!strcmp(com, "#block"))
			{
				if (Result[pos] != '(')return pos;
				char FL[128];
				char ID[64];
			...
			else
			{
				...
				for (int i = 0; i < N_IFNS; i++)
				{
					if (!strcmp(IFNS[i].Name, com))
					{
						...
						IFNS[i].AddInterface(EXP, DSS, &IFS.x, &IFS.y, &IFS.x1, &IFS.y1, IFS.NActiveRefs, IFS.ActiveRefs, IFS.NParams,
						                     IFS.Params, IFNS[i].param);
						int NAfter = 0;
						if (DSS)
						{
						...
						if (CURBOX)
						{
							...
							{
								CURBOX->MaxISPOS += 32;
								CURBOX->ISPOS = (InterfaceElementPosition*)realloc(CURBOX->ISPOS,
								                                                   CURBOX->MaxISPOS * sizeof InterfaceElementPosition);
							};
...
extern "C" __declspec( dllexport ) void SXP_SetVar(int Index, char* Name, char* value)
{
	...
	for (int i = 0; i < EXP->NGVars; i++)
		if (!strcmp(EXP->GVARS[i].Name, Name))
		{
...
void OneSicWindow::ReParse()
{
	...
	for (int i = 0; i < NBOX; i++)
	{
		for (int j = 0; j < NBoxes; j++)
			if (!strcmp(Boxes[j].Name, BOXES[i]))
			{
				if (Boxes[j].VS)
				{
