...
__declspec( dllexport ) int Process_GSC_ChatWindow(bool Active, RoomInfo* RIF)
{
	...
	sscanf(INI.GetStr("PLLIST_ACTIVE_CORNERS"), "%d%d%d%d%d%d%d%d", PXA, PXA + 1, PXA + 2, PXA + 3, PXA + 4, PXA + 5,
	       PXA + 6, PXA + 7);
	sscanf(INI.GetStr("PLLIST_PASSIVE_CORNERS"), "%d%d%d%d%d%d%d%d", PXP, PXP + 1, PXP + 2, PXP + 3, PXP + 4, PXP + 5,
	       PXP + 6, PXP + 7);
	...
	GP_Button* IE_REFR = DSS.addGP_Button(nullptr, INI.GetInt("EXPLORER_REFRESH_BUTTON_X"),
	                                      INI.GetInt("EXPLORER_REFRESH_BUTTON_Y"), ISB.GPID, 2, 3);
	IE_REFR->OnUserClick = &MMItemChoose;
	IE_REFR->UserParam = 108;
	IE_REFR->Visible = 0 != INI.GetInt("EXPLORER_REFRESH_BUTTON_VISIBLE");

	GP_Button* IE_FORW = DSS.addGP_Button(nullptr, INI.GetInt("EXPLORER_FORWARD_BUTTON_X"),
	                                      INI.GetInt("EXPLORER_FORWARD_BUTTON_Y"), ISB.GPID, 4, 5);
	IE_FORW->OnUserClick = &MMItemChoose;
	IE_FORW->UserParam = 109;
	IE_FORW->Visible = 0 != INI.GetInt("EXPLORER_FORWARD_BUTTON_VISIBLE");

	GP_Button* IE_HOME = DSS.addGP_Button(nullptr, INI.GetInt("EXPLORER_HOME_BUTTON_X"),
	                                      INI.GetInt("EXPLORER_HOME_BUTTON_Y"), ISB.GPID, 6, 7);
	...
	GP_Button* IE_BTLS = DSS.addGP_Button(nullptr, INI.GetInt("EXPLORER_BATTLES_BUTTON_X"),
	                                      INI.GetInt("EXPLORER_BATTLES_BUTTON_Y"), ISB.GPID, 8, 9);
	...
	GP_Button* IE_T100 = DSS.addGP_Button(nullptr, INI.GetInt("EXPLORER_TOP100_BUTTON_X"),
	                                      INI.GetInt("EXPLORER_TOP100_BUTTON_Y"), ISB.GPID, 26, 27);
	...
	GP_Button* IE_SZUP = DSS.addGP_Button(nullptr, INI.GetInt("EXPLORER_RESIZE_X"), INI.GetInt("EXPLORER_RESIZE_Y"),
	                                      ISB.GPID, 16, 17);
	IE_SZUP->OnUserClick = &MMItemChoose;
	IE_SZUP->UserParam = 113;
	IE_SZUP->Hint = GetTextByID("VIEW_RESIZE");

	GP_Button* IE_SZDN = DSS.addGP_Button(nullptr, INI.GetInt("EXPLORER_RESIZE_X"), INI.GetInt("EXPLORER_RESIZE_Y"),
	                                      ISB.GPID, 18, 19);
	IE_SZDN->OnUserClick = &MMItemChoose;
	IE_SZDN->UserParam = 114;
	IE_SZDN->Hint = GetTextByID("VIEW_RESIZE");

	GP_Button* CH_COMN = DSS.addGP_Button(nullptr, INI.GetInt("CHAT_SWITCH_BUTTON_X"), INI.GetInt("CHAT_SWITCH_BUTTON_Y"),
	                                      ISB.GPID, 10, 11);
	CH_COMN->OnUserClick = &MMItemChoose;
	CH_COMN->UserParam = 105;
	CH_COMN->Hint = GetTextByID("CHAT_ALL");
	GP_Button* CH_PSNL = DSS.addGP_Button(nullptr, INI.GetInt("CHAT_SWITCH_BUTTON_X"), INI.GetInt("CHAT_SWITCH_BUTTON_Y"),
	                                      ISB.GPID, 12, 13);
	CH_PSNL->OnUserClick = &MMItemChoose;
	CH_PSNL->UserParam = 106;
	CH_PSNL->Hint = GetTextByID("CHAT_PERSONAL");

	GP_Button* IE_AVER = DSS.addGP_Button(nullptr, INI.GetInt("CHAT_VERSION_SWITCH_X"),
	                                      INI.GetInt("CHAT_VERSION_SWITCH_Y"), ISB.GPID, 30, 31);
	IE_AVER->OnUserClick = &MMItemChoose;
	IE_AVER->UserParam = 103;
	IE_AVER->Hint = GetTextByID("CHAT_COMMON");

	GP_Button* IE_CVER = DSS.addGP_Button(nullptr, INI.GetInt("CHAT_VERSION_SWITCH_X"),
	                                      INI.GetInt("CHAT_VERSION_SWITCH_Y"), ISB.GPID, 32, 33);
	IE_CVER->OnUserClick = &MMItemChoose;
	IE_CVER->UserParam = 104;
	IE_CVER->Hint = GetTextByID("CHAT_VERSION");

	GP_Button* PL_INFO = DSS.addGP_Button(nullptr, INI.GetInt("PLAYER_INFO_BUTTON_X"), INI.GetInt("PLAYER_INFO_BUTTON_Y"),
	                                      ISB.GPID, 22, 23);
	PL_INFO->OnUserClick = &MMItemChoose;
	PL_INFO->UserParam = 117;
	PL_INFO->Hint = GetTextByID("INHIN2");

	GP_Button* PL_CHAT = DSS.addGP_Button(nullptr, INI.GetInt("PERSONAL_CHAT_BUTTON_X"),
	                                      INI.GetInt("PERSONAL_CHAT_BUTTON_Y"), ISB.GPID, 24, 25);
	PL_CHAT->OnUserClick = &MMItemChoose;
	PL_CHAT->UserParam = 100;
	PL_CHAT->Hint = GetTextByID("INHIN4");

	GP_Button* PL_HEAR = DSS.addGP_Button(nullptr, INI.GetInt("PLAYER_MUTE_X"), INI.GetInt("PLAYER_MUTE_Y"), ISB.GPID, 28,
	                                      29);
	PL_HEAR->OnUserClick = &MMItemChoose;
	PL_HEAR->UserParam = 101;
	PL_HEAR->Hint = GetTextByID("INHIN6");

	GP_Button* PL_MUTE = DSS.addGP_Button(nullptr, INI.GetInt("PLAYER_MUTE_X"), INI.GetInt("PLAYER_MUTE_Y"), ISB.GPID, 20,
	                                      21);
	PL_MUTE->OnUserClick = &MMItemChoose;
	PL_MUTE->UserParam = 102;
	PL_MUTE->Hint = GetTextByID("INHIN5");

	GP_Button* PL_JOIN = DSS.addGP_Button(nullptr, INI.GetInt("JOIN_TO_PLAYER_X"), INI.GetInt("JOIN_TO_PLAYER_Y"),
	                                      ISB.GPID, 14, 15);
	...
	if (Active)
	{
		GP_Button* CANCEL = DSS.addGP_Button(nullptr, INI.GetInt("CHAT_CANCEL_BUTTON_X"), INI.GetInt("CHAT_CANCEL_BUTTON_Y"),
		                                     ISB.GPID, INI.GetInt("CHAT_CANCEL_BUTTON_STATE1"),
		                                     INI.GetInt("CHAT_CANCEL_BUTTON_STATE2"));
		CANCEL->OnUserClick = &MMItemChoose;
		CANCEL->UserParam = mcmCancel;
	...
	{
		GP_Button* CANCEL = DSS.addGP_Button(nullptr, INI.GetInt("CHAT_CANCEL_BUTTON_X"), INI.GetInt("CHAT_CANCEL_BUTTON_Y"),
		                                     ISB.GPID, INI.GetInt("CHAT_CANCEL_BUTTON_STATE1") + 2,
		                                     INI.GetInt("CHAT_CANCEL_BUTTON_STATE2") + 2);
		CANCEL->OnUserClick = &MMItemChoose;
		CANCEL->UserParam = mcmCancel;
