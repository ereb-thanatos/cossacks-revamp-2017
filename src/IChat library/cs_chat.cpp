...
void ChannelMessage(CHAT chat, const char* channel, const char* user, const char* message, int type, void* param)
{
	...
	if (CC)
	{
		for (int q = 0; q < NCHNL; q++)
			if (!strcmp(CC->chatChannel[q], channel))
			{
				CC->CheckMessage((char*)message);
				if (!strcmp(message, "@@@NFO \\$flags$\\g\\"))
				{
...
void UserParted(CHAT chat, const char* channel, const char* user, int why, const char* reason, const char* kicker,
                void* param)
{
	ChatSystem* CC = (ChatSystem*)param;
	if (CC)
...
void EnumUsersCallback(CHAT chat, CHATBool success, const char* channel, int numUsers, const char** users, int* modes,
                       void* param);
...
void GetUserInfoCallback(CHAT chat, CHATBool success, const char* nick, const char* user, const char* name,
	...
	if (CC)
	{
		...
		else
		{
			...
			for (int i = 0; i < CC->NAbsPlayers; i++)
				if (!strcmp(CC->AbsPlayers[i].Nick, nick))
				{
...
bool ChatSystem::ConnectToChat(char* Nick, char* Info, char* Mail, char* Chat)
{
	...
	chat = chatConnect(serverAddress, port, chatNick, chatUser, chatName, &globalCallbacks, NickErrorCallback,
	                   ConnectCallback, this, CHATFalse);
	int t0 = current_time();
	do
	{
		ProcessMessages();
